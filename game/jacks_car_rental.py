...
class JacksCarRental(Game):
    ...
    def one_move(self, action):
        new_state = (self._state[0]-action, self._state[1]+action)
        ...
        new_state = (self._state[0]-cars_rented_loc1, self._state[1]-cars_rented_loc2)
        self._state = new_state

        new_state0 = min(self.parking_cap, self._state[0] + np.random.poisson(self.lambda_return_loc1, 1)[0])
        new_state1 = min(self.parking_cap, self._state[1] + np.random.poisson(self.lambda_return_loc2, 1)[0])
        self._state = (max(0, new_state0), max(0, new_state1) )
