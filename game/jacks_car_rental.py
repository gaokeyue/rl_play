...
class JacksCarRental(Game):
    ...
    def __init__(self):
        ...
        self._state = (randint(0, self.parking_cap), randint(0, self.parking_cap))
    ...
    def one_move(self, action):
        ...
        cars_rented_loc1 = poisson(self.lambda_rent_loc1)
        cars_rented_loc1 = min(cars_rented_loc1, self._state[0])
        cars_rented_loc2 = poisson(self.lambda_rent_loc2)
        ...
        cars_returned_loc1 = poisson(self.lambda_return_loc1)
        cars_left_loc1 = self._state[0] + cars_returned_loc1
        cars_returned_loc2 = poisson(self.lambda_return_loc2)
    ...
    def half_move(self, action, state=None):
        if state is None:
            state = self._state
        after_state = (state[0] - action, state[1] + action)
        return after_state

    def half_move_reverse(self, after_state, state=None):
        if state is None:
            state = self._state
        action =  state[0] - after_state[0]
        return action
...
if __name__ == '__main__':
    ...
    #     action = choice(jackCarRental.available_actions(state))
